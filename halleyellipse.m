function out = halleyellipse(f,df,ddf,initial,maxiter, rectboundaries, roots)
%starting variables
counter=0;
...
%         out=counter; break;
        comp=abs(initial*ones(1,length(roots))-roots) < 10^-1;
        out=(find(comp)); break;
    end
    

    %iterate
    iterdirection=2*f(initial)*df(initial)/(2*df(initial)^2-f(initial)*ddf(initial));
